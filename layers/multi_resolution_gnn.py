...
class MRGNN(nn.Module):
    ...
    This module is adapted from MedGNN to enable graph-based feature refinement
    across multiple resolutions. Each resolution maintains its own graph structure.
    """
    ...
    def forward(self, x):
        """
        Args:
            x: list of tensors, each (B, enc_in, d_model)
        Returns:
            res: list of refined features for each resolution
            adjacency_matrix_list: learned adjacency matrices
        """
        res = []
        adjacency_matrix_list = []
        for l in range(self.res_num):
            out, adjacency_matrix = self.mr_graphs[l](x[l])
            # out is (B, d_model, enc_in), transpose back to (B, enc_in, d_model)
            out = out.permute(0, 2, 1)
            res.append(out)
            adjacency_matrix_list.append(adjacency_matrix)
        
        return res, adjacency_matrix_list
